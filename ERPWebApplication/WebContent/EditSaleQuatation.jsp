...
ArrayList<QuatationSale_model> q_list=pd.viewSalesQuatationInfo(Integer.parseInt(e_id));
int k=1;
String db_status=q_list.get(0).getStatus();
%>
   <head>
...
<form name="quotation_form" action="SalesUpdateController" method="post" >
	      <div class="span12">   
                        ...
                        <h1>&nbsp;&nbsp;&nbsp;Quotations/<%=q_list.get(0).getPrefix() %><%=q_list.get(0).getQuotationsale_id() %></h1>
                        </div>
                        <div class="block">                  
<header>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<div style="margin-left: 65%;">
		...
		<input type="radio" name="status" value="Sales Order"  <%if(q_list.get(0).getStatus().equalsIgnoreCase("Sales Order")){ %> checked="checked" <%}else{ %> disabled="disabled" <%}  %> />Sales Order
					...
					<div class="sec-box">
							...
							<tr>
								<td></td>
									...
									<td>
										<select id="s2_1" style="width: 100%" name="customer" class="form-control">
											<option selected>--Select Customer--</option>
											<%
												customer_model ss= new customer_model();
												GenericsDao<customer_model> g_obj1=new GenericsImp<customer_model>();
											List<customer_model> s_list=g_obj1.viewData(ss);
												for(int i=0;i<s_list.size();i++){
											%>
											<option value=<%=s_list.get(i).getCustomerid() %><%if(Integer.toString(s_list.get(i).getCustomerid()).equals(Integer.toString(q_list.get(0).getCustomer()))){ %> selected<%} %>><%=s_list.get(i).getCustomername()%></option>
											<% }%>
										</select>
									</td>
								...
								<tr>
									...
									<td><input type="text" name="cref" class="form-control" value="<%=q_list.get(0).getCustomer_ref()%>"/></td><td></td>
									<td></td>
									<td></td>
								</tr>
		...
		<!-- </form> -->
                          <div class="contents boxpadding">
                               ...
                               <table id="theBody" border="4" class="table table-condensed">
                               		...
                               		<%
                               			ArrayList<QuatationSaleDetail_model> qd_list=pd.viewQuatationSaleDetailInfo(q_list.get(0).getQuotationsale_id());
                               		...
                               		<tr>
                               			...
                               			<td><input type="text" value="<%=qd_list.get(i).getTaxs()%>" style="width: 115px;height:30px;text-align: right;" name="tax"><input type="hidden" name="edit_id" value="<%=qd_list.get(i).getDetailsale_id() %>"></td>
                           		...
                           		<table style="margin-left: 25px;" cellpadding="10" cellspacing="10">
									...
									<td>
										<select name="sperson" style="width:70%" class="form-control">
		                            		...
		                            		Integer id11 = (Integer) session.getAttribute("id1");
		                            			...
		                            			List<UserModel> u_list=ua.viewUserData(id11, 0);
											if(u_list.isEmpty())
											{
											%>
											<option value="0"></option>
                            		...
                            		<option value=<%=u_list.get(0).getUserid() %><%if(Integer.toString(u_list.get(0).getUserid()).equals(Integer.toString(q_list.get(0).getResponsible()))){ %> selected <%} %>><%=u_list.get(0).getUsername() %></option>
                            		<%} %>
										</select>
									</td>
								...
								<tr>
									<td>Payment Term</td>
									<td>
										<select name="pterm" style="width:70%" class="form-control">
											<option selected value="0"></option>
											<option value="15 Days" <%if(q_list.get(0).getPaymentterm().equalsIgnoreCase("15 Days")){ %> selected <%} %>>15 Days</option>
											<option value="30 Net Days" <%if(q_list.get(0).getPaymentterm().equalsIgnoreCase("30 Net Days")){ %> selected <%} %>>30 Net Days</option>
											<option value="30% Advance End 30 Days" <%if(q_list.get(0).getPaymentterm().equalsIgnoreCase("30% Advance End 30 Days")){ %> selected <%} %>>30% Advance End 30 Days</option>
											<option value="Immediate Payment" <%if(q_list.get(0).getPaymentterm().equalsIgnoreCase("Immediate Payment")){ %> selected <%} %>>Immediate Payment</option>
										</select>
									</td>
								</tr>
									 ...
									 <td>
										<table  cellpadding="15" cellspacing="15" class="table table-condensed" style="border-spacing: 100px 100px;">
											<tr>
												<td>Shipping Policy</td>
												<td>
													<select name="spolicy" style="width:70%" class="form-control">
											<option selected value="0"></option>
											<option value="Deliver each product when available" <%if(q_list.get(0).getShippingmethod().equalsIgnoreCase("Deliver each product when available")){ %> selected <%} %>>Deliver each product when available</option>
											<option value="Deliver each product at once" <%if(q_list.get(0).getShippingmethod().equalsIgnoreCase("Deliver each product at once")){ %> selected <%} %>>Deliver each product at once</option>
										</select>
												</td>
											</tr>
											<tr>
											<td>Create Invoice</td><td>
												<select name="cinvoice" style="width:70%" class="form-control">
													<option value="0"></option>
													<option value="On Demand" <%if(q_list.get(0).getCreateinvoice().equalsIgnoreCase("On Demand")){ %> selected <%} %>>On Demand</option>
													<option value="On Delivery Order" <%if(q_list.get(0).getCreateinvoice().equalsIgnoreCase("On Delivery Order")){ %> selected <%} %>>On Delivery Order</option>
													<option value="Before Delivery" <%if(q_list.get(0).getCreateinvoice().equalsIgnoreCase("Before Delivery")){ %> selected <%} %>>Before Delivery</option>
												</select>
											</td>
											</tr>
