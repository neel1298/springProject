...
function clickrow(id)
{
	...
	window.document.location="UserController?action=IncomingShippments&id="+id;
}
...
List<IncomingProduct_model> l=new ArrayList<IncomingProduct_model>();
List<PurchaseOrder_model> l2=new ArrayList<PurchaseOrder_model>();
	l=p.viewIncomingProductByid(id11);
	l2=p.viewPurchseOrderByid(id11);%>
			...
			<div class="row">
				<div class="col-xs-12">
					<div class="sec-box">
						...
						<header>
							<h2 class="heading">
								Incoming Products</br> </br> <input type="button"
									class="form-control btn btn-primary text-center"
									style="width: 65px; height: 30px" value="Create" onClick="f1()" />
								</td>
							</h2>
						</header>
						<div class="contents">
							...
							<div class="table-box">
								...
								<table class="table table-condensed" id="example">
									...
									<tbody>
										<%
										request.getSession().setAttribute("status", "Available");
											int k=1;
										for(IncomingProduct_model pp:l)
										{
										%>
										<%
											ArrayList<Supplier_model> sa=p.viewSupplierInfo(l2.get(0).getSupplier());
											PurchaseDaoImp p11=new PurchaseDaoImp();									
										%>
										<tr onclick="clickrow(<%=p11.getPOID(pp.getIncomingshippment_id())%>);"
											style="cursor: pointer" class="gradeX">

											<td><%=pp.getSchedule_dt()%></td>
											<td><%=l2.get(0).getPrefix()%><%=pp.getIncomingshippment_id()%>
											</td>
											<td><%=sa.get(0).getSuppliername()%></td>
											<td class="center"></td>
									...
									<tfoot>
										<tr>
											<th><input type="text" name="search_engine"
												value="Search Reference" class="search_init" /></th>
											<th><input type="text" name="search_browser"
												value="Search Supplier " class="search_init" /></th>
											<th><input type="text" name="search_platform"
												value="Search Order Date" class="search_init" /></th>
											<th><input type="text" name="search_version"
												value="Search Expected Date" class="search_init" /></th>
											<th><input type="text" name="search_version"
												value="Search Status" class="search_init" /></th>
										</tr>
									</tfoot>
									<tr>
										<td><input type="hidden" name="cid"></td>
									</tr>
								</table>
								<script>
