...
System.out.println("pids"+e_id+ " " +st);
PurchaseDao pd=new PurchaseDaoImp();
PurchaseDaoImp pp1=new PurchaseDaoImp();
ArrayList<IncomingProduct_model> q_list=pp1.viewIncomingProductInfo2(Integer.parseInt(e_id),st);
int k=1;
%>
   <head>
 ...
 function some(o){
	 ...
	 function getSizeOld(id){
  	...
  	function FBC(){
		alert('Status is Done or Received');
		parent.$.fancybox.close();
		return true;
...
font-size: 20px;
			...
			<%if(request.getParameter("status").equalsIgnoreCase("received")||request.getParameter("status").equalsIgnoreCase("done")){
			%><body onload="FBC()"><%	
			}
			else{
				%>
			
			<form name="quotation_form" action="ReceiveController" method="post" >
	   			   <div class="span12">   
		       ...
		       <table id="theBody" border="4" class="table table-condensed">
                               		...
                               		<%
                               			ArrayList<IncomingProduct_model> qd_list=pp1.viewIncomingProductInfo2(q_list.get(0).getIncomingshippment_id(),st);
                               		...
                               		<tr>
                               	<td> <input type="submit" class="btn btn-danger text-center"  style="width: 120px; height: 30px" value="Receive Products" name="btnClick" onClick="return FBClose()"/></td>
       
        
 <td> &nbsp;&nbsp;or<a href="NewHome.jsp?p=IncomingShippments&id=<%=qd_list.get(0).getIncomingshippment_id()%>" style="margin-left:10px;" onClick="return FBClose()" >Discard</a></td>
