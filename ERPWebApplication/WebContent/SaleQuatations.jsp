...
function f1()
{
	...
	ff.action="NewHome.jsp?p=AddSaleQuatation";
...
{
	window.document.location="UserController?action=ViewSaleQuatation&id="+id;
}
...
List<QuatationSale_model> l=new ArrayList<QuatationSale_model>();
                                ...
                                <div class="contents">
                                    ...
                                    <div class="table-box">
                                        ...
                                        <table class="table table-condensed" id="example">
                                            ...
                                            <tbody>
                                            	<% int k=1;
for(QuatationSale_model pp:l)
{
%>											<%
													List<customer_model> sa=aa.viewCustomerBycustid(pp.getCustomer());
%>
                                                <tr onclick="clickrow(<%=pp.getQuotationsale_id()%>);"  style="cursor: pointer" class="gradeX">
                                             
                                                    <td><%=pp.getPrefix()%><%=pp.getQuotationsale_id() %></td>
                                                   <td><%=sa.get(0).getCustomername() %>
                                         			</td>
                                         			<td><%=pp.getOrder_dt() %></td>
                                                    <td class="center"><%=pp.getTotal()%></td>
                                            ...
                                            <tfoot>
                                                <tr>
                                                    ...
                                                    <th><input type="text" name="search_browser" value="Search Customer " class="search_init" /></th>
                                                    <th><input type="text" name="search_platform" value="Search Order Date" class="search_init" /></th>
                                                    <th><input type="text" name="search_version" value="Search Total" class="search_init" /></th>
